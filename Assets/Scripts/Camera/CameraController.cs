...
public class CameraController : MonoBehaviour
{
    ...
    public void Init()
    {
        ...
        cameraY = currentTarget.eulerAngles.x;
        cameraX = currentTarget.eulerAngles.y;
    ...
    public void SetMainTarget(Transform newTarget)
    {
        ...
        cameraY = currentTarget.rotation.eulerAngles.x;
        cameraX = currentTarget.rotation.eulerAngles.y;
        Init();
    }
    ...
    {
        // free rotation 
        cameraX += x * xMouseSensitivity;
        cameraY -= y * yMouseSensitivity;

        movementSpeed.x = x;
        movementSpeed.y = -y;
        if (!lockCamera)
        {
            cameraY = CameraExtensions.ClampAngle(cameraY, yMinLimit, yMaxLimit);
            cameraX = CameraExtensions.ClampAngle(cameraX, xMinLimit, xMaxLimit);
        }
        else
        {
            cameraY = currentTarget.root.localEulerAngles.x;
            cameraX = currentTarget.root.localEulerAngles.y;
        }
    }
    ...
    void CameraMovement()
    {
        ...
        Quaternion newRot = Quaternion.Euler(cameraY, cameraX, 0);
